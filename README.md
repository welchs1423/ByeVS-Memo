...
- ✨ [Easter Egg] 타격감 모드, 해커 테마, 커스텀 상용구 추가
- **Document Outline Panel**: Added a collapsible "개요" toggle button in the top panel. When enabled, a 200 px side panel appears to the right of the editor, listing all Markdown headings (`#`, `##`, `###`, etc.) extracted from the current text. Each heading is shown with indentation proportional to its level; `#` headings appear bold at 13 px, `##` at 12 px, `###` at 11 px. Clicking any entry scrolls `MainTextBox` to the corresponding line and places the caret there. The outline refreshes automatically on every keystroke via `MainTextBox_TextChanged`. Panel colors adapt to dark/light theme via `ApplyOutlineTheme()`. Implemented with a `ListBox` (`OutlineListBox`) inside a `Border` in a new Column 4 of the editor grid, with a `GridSplitter` in Column 3.
- **Smart Text Snippets**: In `MainTextBox`, typing a shorthand followed by `Space` automatically expands it: `/date` → current date and time (e.g. `2026-04-11 15:30`), `/todo` → `[ ] ` (checkbox prefix), `/sign` → `- 작성자: [system username] -`. The caret is placed immediately after the expanded text. Implemented via `PreviewKeyDown` on `MainTextBox`; the event is marked handled to suppress the triggering space.
